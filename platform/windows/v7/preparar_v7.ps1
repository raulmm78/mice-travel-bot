...
$expectedHash = "63B1521E6F565FDB732AC87A1225E3095BAE6A3A4084EA3DC5655AFDBF0DB239"
...
try {
    if (-not (Test-Path $target) -or -not (Test-Path $source)) {
        throw "Faltan archivos del bot o de la version v7. Espera a que OneDrive sincronice."
    }
    if ((Get-PortableHash $source) -ne $expectedHash) {
        throw "La version v7 no se ha sincronizado correctamente."
    }
    ...
    foreach ($port in 8765..8784) {
        ...
        if ($status.phase -match "Guardando|Extrayendo|Importando|Comprobando") {
            throw "El bot anterior sigue procesando ($($status.phase)). Espera antes de abrir v7."
        }
    ...
    if ((Get-PortableHash $target) -ne $expectedHash) {
        Write-Host "Aplicando MICE Travel Bot v7..."
        & (Join-Path $PSScriptRoot "instalar_v7.ps1")
        if ($LASTEXITCODE -ne 0) { throw "No se ha completado la actualizacion v7." }
    }
    Write-Host "MICE Travel Bot v7 listo."
} catch {
