...
$log = Join-Path $root "INSTALACION_MICE_TRAVEL_BOT_v7.log"
...
$expectedCodeHash = "63B1521E6F565FDB732AC87A1225E3095BAE6A3A4084EA3DC5655AFDBF0DB239"
...
"Instalacion v7 - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Set-Content -Path $log -Encoding UTF8
try {
    $listeningBotPorts = 8765..8784 | Where-Object {
        Get-NetTCPConnection -LocalPort $_ -State Listen -ErrorAction SilentlyContinue
    }
    if ($listeningBotPorts) {
        throw "El panel sigue abierto. Cierra MICE Travel Bot antes de instalar v7."
    }
    ...
    if ((Get-PortableHash $codeSource) -ne $expectedCodeHash) {
        throw "El codigo v7 no coincide con el archivo esperado. No se ha instalado."
    }
    if ((Get-PortableHash $launcherSource) -ne $expectedLauncherHash) {
        throw "El lanzador v7 no coincide con el archivo esperado. No se ha instalado."
    }
    ...
    "OK: MICE TRAVEL BOT v7 instalado y comprobado." | Add-Content -Path $log
    Write-Host "OK: MICE TRAVEL BOT v7 instalado y comprobado."
} catch {
