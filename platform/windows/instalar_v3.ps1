...
$log = Join-Path $root "INSTALACION_MICE_TRAVEL_BOT_v3.log"
...
$expectedCodeHash = "AEDF0AD697242E268D53BE8B644AFE4B8F028F942E4A8FFF7A43D38CD527D5F1"
...
try {
    ...
    if ((Get-PortableHash $codeSource) -ne $expectedCodeHash) {
        throw "El codigo v3 no coincide con el archivo esperado. No se ha instalado."
    }
    if ((Get-PortableHash $launcherSource) -ne $expectedLauncherHash) {
        throw "El lanzador v3 no coincide con el archivo esperado. No se ha instalado."
    }
    ...
    "OK: MICE TRAVEL BOT v3 instalado y comprobado." | Add-Content -Path $log
    Write-Host "OK: MICE TRAVEL BOT v3 instalado y comprobado."
} catch {
