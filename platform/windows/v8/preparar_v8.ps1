...
$expectedHash = "145761663152E5A4BE35DA7D0A81B30F85044196BC11405000B6ECF4692734F8"
...
try {
    if (-not (Test-Path $target) -or -not (Test-Path $source)) {
        throw "Faltan archivos del bot o de la version v8. Espera a que OneDrive sincronice."
    }
    if ((Get-PortableHash $source) -ne $expectedHash) {
        throw "La version v8 no se ha sincronizado correctamente."
    }
    ...
    foreach ($port in 8765..8784) {
        ...
        if ($status.phase -match "Guardando|Extrayendo|Importando|Comprobando") {
            throw "El bot anterior sigue procesando ($($status.phase)). Espera antes de abrir v8."
        }
    ...
    if ((Get-PortableHash $target) -ne $expectedHash) {
        Write-Host "Aplicando MICE Travel Bot v8..."
        & (Join-Path $PSScriptRoot "instalar_v8.ps1")
        if ($LASTEXITCODE -ne 0) { throw "No se ha completado la actualizacion v8." }
    }
    Write-Host "MICE Travel Bot v8 listo."
} catch {
