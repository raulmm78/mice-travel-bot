...
$log = Join-Path $root "INSTALACION_MICE_TRAVEL_BOT_v8.log"
...
$expectedCodeHash = "145761663152E5A4BE35DA7D0A81B30F85044196BC11405000B6ECF4692734F8"
...
"Instalacion v8 - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Set-Content -Path $log -Encoding UTF8
try {
    $listeningBotPorts = 8765..8784 | Where-Object {
        Get-NetTCPConnection -LocalPort $_ -State Listen -ErrorAction SilentlyContinue
    }
    if ($listeningBotPorts) {
        throw "El panel sigue abierto. Cierra MICE Travel Bot antes de instalar v8."
    }
    ...
    if ((Get-PortableHash $codeSource) -ne $expectedCodeHash) {
        throw "El codigo v8 no coincide con el archivo esperado. No se ha instalado."
    }
    if ((Get-PortableHash $launcherSource) -ne $expectedLauncherHash) {
        throw "El lanzador v8 no coincide con el archivo esperado. No se ha instalado."
    }
    ...
    "OK: MICE TRAVEL BOT v8 instalado y comprobado." | Add-Content -Path $log
    Write-Host "OK: MICE TRAVEL BOT v8 instalado y comprobado."
} catch {
