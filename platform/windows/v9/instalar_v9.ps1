...
$log = Join-Path $root "INSTALACION_MICE_TRAVEL_BOT_v9.log"
...
$expectedCodeHash = "63419EF5F2821077395260C584FB0AE5E35740B29A27CF46F6ABD13B7EF2519E"
...
"Instalacion v9 - $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')" | Set-Content -Path $log -Encoding UTF8
try {
    $listeningBotPorts = 8765..8784 | Where-Object {
        Get-NetTCPConnection -LocalPort $_ -State Listen -ErrorAction SilentlyContinue
    }
    if ($listeningBotPorts) {
        throw "El panel sigue abierto. Cierra MICE Travel Bot antes de instalar v9."
    }
    ...
    if ((Get-PortableHash $codeSource) -ne $expectedCodeHash) {
        throw "El codigo v9 no coincide con el archivo esperado. No se ha instalado."
    }
    if ((Get-PortableHash $launcherSource) -ne $expectedLauncherHash) {
        throw "El lanzador v9 no coincide con el archivo esperado. No se ha instalado."
    }
    ...
    "OK: MICE TRAVEL BOT v9 instalado y comprobado." | Add-Content -Path $log
    Write-Host "OK: MICE TRAVEL BOT v9 instalado y comprobado."
} catch {
