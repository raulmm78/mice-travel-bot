...
$expectedHash = "63419EF5F2821077395260C584FB0AE5E35740B29A27CF46F6ABD13B7EF2519E"
...
try {
    if (-not (Test-Path $target) -or -not (Test-Path $source)) {
        throw "Faltan archivos del bot o de la version v9. Espera a que OneDrive sincronice."
    }
    if ((Get-PortableHash $source) -ne $expectedHash) {
        throw "La version v9 no se ha sincronizado correctamente."
    }
    ...
    foreach ($port in 8765..8784) {
        ...
        if ($status.phase -match "Guardando|Extrayendo|Importando|Comprobando") {
            throw "El bot anterior sigue procesando ($($status.phase)). Espera antes de abrir v9."
        }
    ...
    if ((Get-PortableHash $target) -ne $expectedHash) {
        Write-Host "Aplicando MICE Travel Bot v9..."
        & (Join-Path $PSScriptRoot "instalar_v9.ps1")
        if ($LASTEXITCODE -ne 0) { throw "No se ha completado la actualizacion v9." }
    }
    Write-Host "MICE Travel Bot v9 listo."
} catch {
